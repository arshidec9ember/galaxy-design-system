...
- css
  - css index `@zeta-gds/components/lib|es/styles/index.css` has been removed, do not import it any more!
- fonts
  - `@zeta-gds/components/lib|es/styles/fonts/*` has been removed, use [vfonts](https://www.npmjs.com/package/vfonts)
- icons

  - `@zeta-gds/components/lib|es/icons/*` is deprecated (vue-loader is required).
  - `@zeta-gds/components/lib|es/icons/*` is removed since v2.10.0
  - use [xicons](https://github.com/07akioni/xicons) instead.

- `z-nimbus-icon` is moved to `@zeta-gds/components/compat/nimbus-icon` (vue-loader is required) `z-nimbus-icon` is removed since v2.10.0
- `z-nimbus-form-card` is removed
- locale & theme on `z-config-provider` doesn't accept string any more (not it accepts object).
...
- [x] auto-complete
  ...
  - deprecate
    - `on-input` => `on-update:model-value`
  - new
    - `on-blur`
    - `on-focus`
- [x] avatar
...
- [x] cascader
  ...
  - new
    - `cascade` prop
    - `show-path` prop
    - `default-model-value` prop
  - deprecated
    - `on-change` => `on-update:model-value`
- [x] checkbox
  - checkbox
    - break
...
- [x] config-provider
  - break
    - `theme` use new theme from zeta-gds export
...
- [x] menu
  ...
  - remove
    ...
    - `overlay-min-width`
- [x] message
  - rewrite message using `z-message-provider`
  - new
    - `useMessage`
  - deprecate
    - `onHide` => `onLeave`
...
- [x] tabs
  - deprecate
    - `active-name` => `value`
    - `on-active-name-change` => `on-update:model-value`
- [x] tag
  - break
    - `v-model` => `v-model:value`
  - deprecate
