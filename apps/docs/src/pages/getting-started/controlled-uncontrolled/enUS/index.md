...
In this situation, you don't set `<z-input />`'s `value` but only listen to its change. Component's value will be controlled by itself.

```html
<z-input @update:model-value="handleUpdateValue" />
```
...
```html
<z-input :model-value="value" @update:model-value="handleUpdateValue" />
```
...
## Uncontrolled manner in @zeta-gds/components

Different library has different behavior on how to distinguish controlled or uncontrolled manner. In @zeta-gds/components, if `value` is `undefined` or not passed, it will be uncontrolled. That is to say, if you set a component's value to `undefined` won't clear it but transform it from controlled manner to uncontrolled manner. If you need to clear it, at most time you can use `null`.
