...
@zeta-gds/components provides `z-config-provider` to customize the theme.

By default all of the components are light themed, without any configuration.

To learn more about `z-config-provider`, see [Config Provider](../components/config-provider).

## Use dark theme

Set `z-config-provider`'s `theme` prop to `darkTheme` imported from @zeta-gds/components to set dark theme inside `z-config-provider`.

If `theme` is `undefined` it won't affect the theme of components inside.

```html
<template>
  <z-config-provider :theme="darkTheme">
    <app />
  </z-config-provider>
</template>
...
<script>
  ...
  import { darkTheme } from '@zeta-gds/components'
...
Set `z-config-provider`'s `theme-overrides` to customize theme vars. @zeta-gds/components exports type `GlobalThemeOverrides` to help you define `theme-overrides`.

For available vars please follow the type hint of `GlobalThemeOverrides`.

If you want to view more theme variables, you can view them in the edit button at the bottom right corner of the Galaxy Design Components homepage.
...
<script>
   ...
   * @type import('@zeta-gds/components').GlobalThemeOverrides
   */
...
<template>
  <z-config-provider :theme-overrides="themeOverrides">
    <my-app />
  </z-config-provider>
</template>
```
...
```html
<script lang="ts">
  import { ZConfigProvider, GlobalThemeOverrides } from '@zeta-gds/components'
...
```html
<script lang="ts">
  import { SelectProps, ButtonProps } from '@zeta-gds/components'
...
<template>
  ...
  />
  <z-button :theme-overrides="buttonThemeOverrides">theme</z-button>
</template>
```
...
```html
<script>
  import { ZConfigProvider, darkTheme } from '@zeta-gds/components'
...
@zeta-gds/components provides a theme editor to help you edit theme and export the corresponding configuration. It can be placed inside `z-config-provider`.

The theme editor is not included in global installation (`app.use(zeta-gds)`). You need to import it explicitly to use it.
...
<script>
  import { defineComponent } from 'vue'
  import { ZThemeEditor } from '@zeta-gds/components'
