...
const __dirname = path.dirname(
  path.join(cwd(), 'galaxy-design-system', 'package', 'components')
)
const baseDir = path.resolve(__dirname, '../..')

export async function genWebTypes () {
  const components = await import('../lib/components.js')
  const { default: version } = await import('../lib/version.js')
  ...
  for (const [exportName, component] of Object.entries(components)) {
    if (exportName[0] !== 'Z') continue
    if (exportName.startsWith('Zx')) continue
  ...
  function loadDocs (componentName) {
    ...
    return {
      ...
      slots,
      description,
      docUrl: `https://gds-docs0zetaapps.mum1-pp.zetaapps.in/en-US/os-theme/components/${componentPath}`
    }
