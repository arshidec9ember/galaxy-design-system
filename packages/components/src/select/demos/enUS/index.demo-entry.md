# Select
The select field component is a part of the form fields, which is used with the Select component. It acts as a trigger point for the popover and is displays the feedback.

```figma
https://www.figma.com/file/t4dWFndlBl46TnLIH2n1dJ/%F0%9F%94%AE-GDS-v2.11.0---Foundation-%2B-Components?type=design&node-id=48700-154522&mode=design&t=hWxYO9X5N4XCUQg0-0
```

```overview-html
select/demos/enUS/index.html
```
...
| default-model-value | `Array<string \| number> \| string \| number \| null` | `null` | Default value. |
| delimiters | `Array<string>` | `,` | Delimiter to seperate text on copy paste. |
...
| ignore-composition | `boolean` | `true` | Ingore IME's composition status. By default `filter` won't be triggered by input event under compositions. |
...
| keyboard | `boolean` | `true` | Whether to allow keyboard control. |
| label-field | `string` | `'label'` | Field name of option label. |
| loading | `boolean` | `false` | Whether to show a loading state. |
| limit-tag | `number` | `undefined` | Max selection that can be done. |
...
| placement | `'top-start' \| 'top' \| 'top-end' \| 'right-start' \| 'right' \| 'right-end' \| 'bottom-start' \| 'bottom' \| 'bottom-end' \| 'left-start' \| 'left' \| 'left-end'` | `'bottom-start'` | Option menu's placement. |
...
| render-option-label | `(option: SelectOption \| SelectGroupOption, selected: boolean) => VNodeChild` | `undefined` | Render function for each option label. |
...
| render-option-tag | `(props: { option: SelectBaseOption, handleClose: () => void }) => VNodeChild` | `undefined` | Render function for each option tag. |
| reset-menu-on-options-change | `boolean` | `true` | Whether to reset menu staus on options change, for example, scroll status. |
| retainOnDuplicate | `boolean` | `true` | The selected item will be remain even though selected again. |
| show | `boolean` | `undefined` | Whether to show/open the option menu. |
| show-arrow | `boolean` | `true` | Whether to show the dropdown arrow. |
| show-checkmark | `boolean` | `true` | Whether to show checkmark in multiple select mode. |
| show-on-focus | `boolean` | `false` | Whether to show menu on focus. |
...
| model-value | `Array<string \| number> \| string \| number \| null` | `undefined` | Value when being manually set. |
...
| on-update:show | `(show: boolean) => void` | `undefined` | Callback on menu open status change. |
| on-update:model-value | `(value: Array \| string \| number \| null, option: SelectBaseOption \| null \| SelectBaseOption[]) => void` | `undefined` | Callback triggered when the selected value is updated. |
