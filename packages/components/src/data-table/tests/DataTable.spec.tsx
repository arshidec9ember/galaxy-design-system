...
import { type DataTableInst, ZDataTable, type DataTableColumns } from '../index'
import { ZButton } from '../../button'
import { ZButtonGroup } from '../../button-group'

describe('z-data-table', () => {
  const testColumns = [
    {
      title: 'Name',
      key: 'name'
  ...
  const testData = new Array(5).fill(0).map((_, i) => ({ name: i }))
  ...
  it('show custom empty', () => {
    ...
    const wrapper = mount(() => (
      <ZDataTable columns={columns} data={[]}>
        {{
  ...
  it('should work with `itemCount` without `remote`', () => {
    ...
    const data = new Array(97).fill(0).map((_, index) => {
      return {
        name: index
      }
    })
    ...
    const wrapper = mount(() => (
      <ZDataTable columns={columns} data={data} pagination={pagination} />
    ))
    expect(wrapper.find('.z-pagination-prefix').text()).toEqual('97')
    wrapper.unmount()
  })
  ...
  it('should work with `itemCount` with `remote`', async () => {
    ...
    let data = new Array(97).fill(0).map((_, index) => {
      return {
        name: index
      }
    })
    ...
    void wrapper.findAll('.z-pagination-item')[2].trigger('click')
    await nextTick()
  ...
  it('should work with `bordered` prop', async () => {
    let wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} />
    ))
    expect(wrapper.find('.z-data-table').classes()).toContain(
      'z-data-table--bordered'
    )
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} bordered={false} />
    ))
    expect(wrapper.find('.z-data-table').classes()).not.toContain(
      'z-data-table--bordered'
    )
    wrapper.unmount()
  })
  ...
  it('should work with `loading` prop', async () => {
    ...
    expect(wrapper.find('.z-base-loading').exists()).not.toBe(true)
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} loading={true} />
    ))
  ...
  it('should work with `flex-height` prop', async () => {
    ...
    expect(wrapper.find('.z-data-table').classes()).not.toContain(
      'z-data-table--flex-height'
    )
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} flexHeight={true} />
    ))
    expect(wrapper.find('.z-data-table').classes()).toContain(
      'z-data-table--flex-height'
    )
  ...
  it('should work with `row-class-name` prop', async () => {
    ...
    expect(wrapper.find('tbody .z-data-table-tr').classes()).not.toContain(
      'test'
    )
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} rowClassName="test" />
    ))
    expect(wrapper.find('tbody .z-data-table-tr').classes()).toContain('test')
    wrapper.unmount()
    ...
    expect(wrapper.find('tbody .z-data-table-tr').classes()).toContain('0-test')
    wrapper.unmount()
  })
  ...
  describe('should work with multiple sorter', () => {
    ...
    const wrapper = mount(
      () => <ZDataTable ref={tableRef} columns={columns} data={data} />,
      {
  ...
  it('should work with `indent` prop', async () => {
    ...
    expect(wrapper.find('.z-data-table-indent').attributes('style')).toContain(
      'width: 16px'
    )
  ...
  it('should work with `row-props` prop', async () => {
    ...
    expect(wrapper.find('tbody .z-data-table-tr').attributes('style')).toBe(
      undefined
    )
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} row-props={rowProps} />
    ))
    expect(
      wrapper.find('tbody .z-data-table-tr').attributes('style')
  ...
  it('should work with `scroll-x` prop', async () => {
    ...
    expect(
      wrapper.find('.z-scrollbar-content').attributes('style')
    ).not.toContain('min-width: 1800px')
    wrapper.unmount()

    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} scroll-x={1800} />
    ))
    expect(wrapper.find('.z-scrollbar-content').attributes('style')).toContain(
      'min-width: 1800px'
    )
    wrapper.unmount()
  })

  it('should work with `hide-row-separator` prop', async () => {
    let wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} />
    ))
    expect(wrapper.find('.z-data-table').classes()).not.toContain(
      'z-data-table--hide-row-separator'
    )
    wrapper.unmount()
    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} row-separator={false} />
    ))
    expect(wrapper.find('.z-data-table').classes()).toContain(
      'z-data-table--hide-row-separator'
    )
  ...
  it('should work with `column-separator` prop', async () => {
    ...
    expect(wrapper.find('.z-data-table').classes()).toContain(
      'z-data-table--hide-column-separator'
    )
    wrapper.unmount()
    wrapper = mount(() => (
      <ZDataTable
  ...
  it('should work with `size` prop', async () => {
    ;(['small', 'medium', 'large'] as const).forEach((size) => {
      const wrapper = mount(() => (
        <ZDataTable columns={testColumns} data={testData} size={size} />
      ))
      expect(
        wrapper.find('.z-data-table').attributes('style')
      ).toMatchSnapshot()
      wrapper.unmount()
    })
  })
  ...
  it('should work with `summary` prop', async () => {
    ...
    const wrapper = mount(() => (
      <ZDataTable columns={columns} data={data} summary={summary} />
    ))
    expect(
      wrapper.findAll('.z-data-table-td--summary')[0].attributes('data-col-key')
    ).toBe('name')
    expect(
      wrapper.findAll('.z-data-table-td--summary')[0].attributes('colspan')
    ).toBe('1')
    expect(
      wrapper.findAll('.z-data-table-td--summary')[0].attributes('rowspan')
    ).toBe('2')
    expect(wrapper.findAll('.z-data-table-td--summary')[0].text()).toBe('45')

    expect(
      wrapper.findAll('.z-data-table-td--summary')[1].attributes('data-col-key')
    ).toBe('age')
    expect(
      wrapper.findAll('.z-data-table-td--summary')[1].attributes('colspan')
    ).toBe('2')
    expect(
      wrapper.findAll('.z-data-table-td--summary')[1].attributes('rowspan')
    ).toBe('1')
    expect(wrapper.findAll('.z-data-table-td--summary')[1].text()).toBe('45')
    wrapper.unmount()
  })
  ...
  it('should work with `table-layout` prop', async () => {
    ...
    wrapper.unmount()
    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={testData} table-layout="fixed" />
    ))
    expect(wrapper.find('table').attributes('style')).toContain(
      'table-layout: fixed'
    )
  ...
  it('should work with `virtual-scroll` prop', async () => {
    ...
    let wrapper = mount(() => <ZDataTable columns={testColumns} data={data} />)
    expect(wrapper.find('tbody').element.children.length).not.toBe(0)
    wrapper.unmount()
    wrapper = mount(() => (
      <ZDataTable columns={testColumns} data={data} virtual-scroll={true} />
    ))
  ...
  it('should work with `on-update:checked-row-keys` prop', async () => {
    ...
    await wrapper.find('.z-checkbox').trigger('click')
...
describe('props.columns', () => {
              ...
              {{
                default: () => {
                  return (
                    ...
                    <ZButton color={type} round borderStyle="dashed">
                      {{ default: () => text }}
                    </ZButton>
                  ))
                }
  ...
  it('should work with `align` prop', async () => {
    ...
    ;(['left', 'right', 'center'] as const).forEach((align) => {
      ...
      const wrapper = mount(() => (
        <ZDataTable columns={columns} data={data} row-key={rowKey} />
      ))
      expect(wrapper.find('tbody .z-data-table-td').classes()).toContain(
        `z-data-table-td--${align}-align`
      )
      expect(
        wrapper.find('tbody .z-data-table-td').attributes('style')
  ...
  it('should work with `rowSpan` `colSpan` prop', async () => {
    ...
    expect(wrapper.find('tbody .z-data-table-td').attributes('colspan')).toBe(
      '3'
    )
    expect(wrapper.find('tbody .z-data-table-td').attributes('rowspan')).toBe(
      '2'
    )
    wrapper.unmount()
  })
  ...
  it('should work with `ellipsis` prop', async () => {
    ...
    expect(wrapper.find('tbody .z-data-table-td__ellipsis').exists()).toBe(true)
    expect(wrapper.find('tbody .z-ellipsis').exists()).toBe(true)
    expect(wrapper.find('tbody .z-ellipsis').attributes('style')).toContain(
  ...
  it('should work with `renderExpand` `expandable` prop', async () => {
    ...
    expect(wrapper.findAll('tbody .z-data-table-td--expand').length).toBe(5)
    ...
    expect(wrapper.findAll('tbody td')[0].attributes('class')).toContain(
      'z-data-table-td--expand'
    )
    void wrapper
      .findAll('tbody .z-data-table-expand-trigger')[0]
      .trigger('click')
    await nextTick()
  ...
  it('should work with `striped` prop', async () => {
    ...
    const wrapper = mount(() => (
      <ZDataTable columns={columns} data={data} striped />
    ))

    const trList = wrapper.find('tbody').findAll('.z-data-table-tr')
    expect(trList[1].attributes('class')).toContain('z-data-table-tr--striped')
    expect(trList[0].attributes('class')).not.toContain(
      'z-data-table-tr--striped'
    )
  ...
  it('should work with `column.multiple` prop', async () => {
    ...
    const radios = wrapper.findAll('.z-radio')

    expect(radios[4].classes()).toContain('z-radio--checked')
    expect(radios[1].classes()).not.toContain('z-radio--checked')

    await radios[1].trigger('click')

    setTimeout(() => {
      expect(radios[1].classes()).toContain('z-radio--checked')
      expect(radios[4].classes()).not.toContain('z-radio--checked')
    }, 0)
    wrapper.unmount()
  })
})
