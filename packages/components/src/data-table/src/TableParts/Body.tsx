...
export default defineComponent({
  ...
  setup (props) {
      ...
      ({ props: cProps }: { props: StyleCProps }) => {
        const createActiveLeftFixedStyle = (
          ...
          return c(
            ...
            { boxShadow: 'var(--z-box-shadow-after)' }
          )
        }
        ...
        const createActiveRightFixedStyle = (
          ...
          return c(
            ...
            { boxShadow: 'var(--z-box-shadow-before)' }
          )
        }
    ...
    return {
      ...
      renderRowExpandIcon: renderExpandIconRef,
  ...
  render () {
          ...
          default: () => {
            ...
            const renderRow = (
              ...
              if ('isExpandedRow' in rowInfo) {
                ...
                return (
                  <tr
                    class={`${mergedClsPrefix}-data-table-tr ${mergedClsPrefix}-data-table-tr--expanded ${mergedClsPrefix}-data-table-tr--expanded-child`}
                    key={`${key}__expand`}
                  >
                    <td
                      class={[
              ...
              const selected = this.mergedCheckedRowKeySet.has(rowKey)
              const disabled = rowInfo.tmNode.disabled
              ...
              const row = (
                <tr
                  ...
                  class={[
                    ...
                    expanded &&
                      `${mergedClsPrefix}-data-table-tr--expanded-parent`,
                    selected && `${mergedClsPrefix}-data-table-tr--selected`,
                    disabled && `${mergedClsPrefix}-data-table-tr--disabled`,
                    mergedRowClassName
                  ]}
                  {...props}
                >
                          ...
                          ? [
                              ...
                              isSummary || rowInfo.tmNode.isLeaf ? (
                                ...
                                <ExpandTrigger
                                  ...
                                  expanded={expanded}
                                  renderRowExpandIcon={this.renderRowExpandIcon}
            ...
            if (!virtualScroll) {
              return (
                ...
                >
                  <colgroup>
                    {cols
                      .filter((c) => c.column.visible !== false)
                      .map((col) => (
                        <col key={col.key} style={col.style}></col>
                      ))}
                  </colgroup>
    ...
    if (this.hasError) {
      const createErrorNode = (): VNode => (
        <div
          class={[
            `${mergedClsPrefix}-data-table-error`,
            this.loading && `${mergedClsPrefix}-data-table-error--hide`
          ]}
          style={this.bodyStyle}
          ref="errorElRef"
        >
          {resolveSlot(this.dataTableSlots.error, () => [
            <ZResult
              variant="image"
              status="4xx"
              title="Error"
              theme={this.mergedTheme.peers.Result}
              themeOverrides={this.mergedTheme.peerOverrides.Result}
            />
          ])}
        </div>
      )
