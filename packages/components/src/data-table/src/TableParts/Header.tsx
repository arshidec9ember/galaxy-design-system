          ...
          return (
            <tr class={`${mergedClsPrefix}-data-table-tr`}>
              {row.map(({ column, colSpan, rowSpan, isLast }) => {
                ...
                const { ellipsis, description, icon } = column
                ...
                const createColumnVNode = (): VNode | null => {
                    ...
                    return column.multiple !== false ? (
                      <>
                        <ZCheckbox
                          key={currentPage}
                          privateInsideTable
                          modelValue={allRowsChecked}
                          indeterminate={someRowsChecked}
                          disabled={headerCheckboxDisabled}
                          onUpdateModelValue={handleCheckboxUpdateChecked}
                        />
                        {checkOptions ? (
                  ...
                  const titleAlign = (column.titleAlign || column.align || '')
                    .trim()
                    .toLowerCase()
                  const columnJustifyContent =
                    justifyContentMap[titleAlign] || justifyContentMap.left
                  return (
                    <>
                      <div
                      ...
                      >
                        {icon && (
                          <span
                            class={`${mergedClsPrefix}-data-table-th__icon`}
                          >
                            {icon()}
                          </span>
                        )}
                      ...
                      <ZText variant="4-r">
                        {description ? (
                          <div
                            class={`${mergedClsPrefix}-data-table-th__description`}
                          >
                            {ellipsis === true ||
                            (ellipsis && !ellipsis.tooltip) ? (
                              <div
                              ...
                              >
                                {{
                                  default: () => renderDescription(column)
                                }}
                              </ZEllipsis>
                                ) : (
                                  renderDescription(column)
