...
import { formatLength, toPX } from '../../_utils'
import type {
  ColumnKey,
  MainTableRef,
  TableColumn,
...
export function useScroll (
  props: DataTableSetupProps,
  {
    ...
    columnsValueRef: Ref<TableColumns<any>>
  }
) {
  ...
  const leftFixedColumnsRef = computed(() => {
    return columnsValueRef.value?.filter((column) => column.fixed === 'left')
  })
  const rightFixedColumnsRef = computed(() => {
    return columnsValueRef.value?.filter((column) => column.fixed === 'right')
  })
  ...
  function deriveActiveLeftFixedChildrenColumns (): void {
    ...
    let activeLeftFixedColumn = columnsValueRef.value?.find(
  ...
  function deriveActiveRightFixedColumn (): void {
    ...
    let parentEl = getScrollElements().body ?? undefined
    if (parentEl) {
      parentEl = parentEl.querySelector('table') ?? undefined
    }
    const numericScrollX = Number(props.scrollX)
    const scrollWidth = Number.isNaN(numericScrollX)
      ? Number(toPX(String(props.scrollX), parentEl))
      : numericScrollX
  ...
  function deriveActiveRightFixedChildrenColumns (): void {
    ...
    let activeRightFixedColumn = columnsValueRef.value?.find(
  ...
  function recalculateFixedPositions (): void {
    deriveActiveLeftFixedColumn()
    deriveActiveLeftFixedChildrenColumns()
    deriveActiveRightFixedColumn()
    deriveActiveRightFixedChildrenColumns()
