...
describe('z-log', () => {
  ...
  it('should work with `font-size` prop', async () => {
    ...
    expect(wrapper.find('.z-code').attributes('style')).toContain(
      `--z-font-size: ${fontSize}px`
    )
    wrapper.unmount()
  })
  ...
  it('should work with `line-height` prop', async () => {
    ...
    expect(wrapper.find('.z-log').attributes('style')).toContain(
  ...
  it('should work with `lines` `log` prop', async () => {
    ...
    expect(wrapper.find('.z-code').element.children.length).toBe(2)
    expect(wrapper.find('.z-code').element.children[0].textContent).toBe(
      'test1'
    )
    expect(wrapper.find('.z-code').element.children[1].textContent).toBe(
      'test2'
    )

    await wrapper.setProps({ log: 'test3' })
    expect(wrapper.find('.z-code').element.children.length).toBe(1)
    expect(wrapper.find('.z-code').element.children[0].textContent).toBe(
      'test3'
    )
    wrapper.unmount()
  ...
  it('should work with `loading` prop', async () => {
    ...
    expect(wrapper.find('.z-log-loader').exists()).toBe(false)

    await wrapper.setProps({ loading: true })
    expect(wrapper.find('.z-log-loader').exists()).toBe(true)
    wrapper.unmount()
  })
