...
export default defineComponent({
  ...
  props: colProps,
  setup (props) {
    const ZRow = inject(rowInjectionKey, null)
    if (!ZRow) throwError('col', '`z-col` must be placed inside `z-row`.')
    return {
      mergedClsPrefix: ZRow.mergedClsPrefixRef,
      gutter: ZRow.gutterRef,
      stylePadding: computed(
        () =>
          `${formatLength(ZRow.verticalGutterRef.value, {
            c: 0.5
          })} ${formatLength(ZRow.horizontalGutterRef.value, { c: 0.5 })}`
      ),
