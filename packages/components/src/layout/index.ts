export {
  default as ZLayout,
  layoutProps,
  layoutProps as layoutContentProps
} from './src/Layout'
export { default as ZLayoutContent } from './src/LayoutContent'
export {
  default as ZLayoutHeader,
  headerProps as layoutHeaderProps
} from './src/LayoutHeader'
export { default as ZLayoutFooter, layoutFooterProps } from './src/LayoutFooter'
export { default as ZLayoutSider, layoutSiderProps } from './src/LayoutSider'
export type {
  LayoutProps,
  LayoutProps as LayoutContentProps
} from './src/Layout'
export type { LayoutHeaderProps } from './src/LayoutHeader'
export type { LayoutFooterProps } from './src/LayoutFooter'
export type { LayoutSiderProps } from './src/LayoutSider'
export type {
  LayoutInst,
  LayoutInst as LayoutContentInst,
  LayoutSiderInst
} from './src/interface'
