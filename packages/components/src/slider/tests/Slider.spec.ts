...
describe('z-slider', () => {
  ...
  it('should work with `defaultModelValue`', async () => {
    const wrapper = mount(ZSlider, {
      props: {
        defaultModelValue: 23
      }
    })

    const sliderRailFill = wrapper.find('.z-slider-rail__fill')
  ...
  it('should work with `marks`', async () => {
    ...
    const sliderDots = wrapper.findAll('.z-slider-dot')
    ...
    const sliderMarks = wrapper.findAll('.z-slider-mark')
  ...
  it('accept correct callback types', () => {
    function onUpdateModelValue1 (value: number): void {}
    function onUpdateModelValue2 (value: number[]): void {}
    mount(ZSlider, {
      props: {
  ...
  it('should work with `disabled` prop', async () => {
    ...
    expect(wrapper.find('.z-slider').classes()).toContain('z-slider--disabled')
    wrapper.unmount()
  })
  ...
  it('should work with `marks` prop', async () => {
    ...
    expect(wrapper.findAll('.z-slider-mark').length).toBe(2)
    expect(wrapper.findAll('.z-slider-mark')[0].attributes('style')).toContain(
      'left: 30%'
    )
    expect(wrapper.findAll('.z-slider-mark')[0].text()).toContain('test1')
    expect(wrapper.findAll('.z-slider-mark')[1].attributes('style')).toContain(
      'left: 70%'
    )
    expect(wrapper.findAll('.z-slider-mark')[1].text()).toContain('test2')
    wrapper.unmount()
  })
  ...
  it('should work with `range` & `defaultModelValue` prop', async () => {
    const wrapper = mount(ZSlider, {
      props: {
        range: true,
        defaultModelValue: [24, 49]
      }
    })

    expect(wrapper.findAll('.z-slider-handle-wrapper').length).toBe(2)
  ...
  it('should work with `vertical` prop', async () => {
    const wrapper = mount(ZSlider, {
      props: {
        defaultModelValue: 77,
        vertical: true
      }
    })

    const sliderRailFill = wrapper.find('.z-slider-rail__fill')
    const firstHandle = wrapper.find('.z-slider-handle-wrapper')
  ...
  it('should work with `range` & `vertical` prop', async () => {
    ...
    expect(
      wrapper.findAll('.z-slider-handle-wrapper')[0].attributes('style')
    ).toContain('bottom: 24%')
    expect(
      wrapper.findAll('.z-slider-handle-wrapper')[1].attributes('style')
    ).toContain('bottom: 49%')
    wrapper.unmount()
  })
  ...
  it('should work with `reverse` prop', async () => {
    ...
    expect(wrapper.find('.z-slider').classes()).not.toContain(
      'z-slider--reverse'
    )

    await wrapper.setProps({ reverse: true })
    expect(wrapper.find('.z-slider').classes()).toContain('z-slider--reverse')
    wrapper.unmount()
  })
  ...
  it('should slided to the specific mark when step is `mark`', async () => {
    ...
    const slider = wrapper.find('.z-slider')
    const handle = wrapper.find('.z-slider-handle-wrapper')
