    ...
    onBeforeMount(() => {
      watchEffect(() => {
        const {
          ...
          ? merge(
            {},
            ZConfigProvider.mergedThemeRef.value?.common || commonLight,
            ZConfigProvider.mergedThemeOverridesRef.value?.common
          )
          : commonLight
        if (styleApplied || !body.hasAttribute('z-styled')) {
          ...
          body.setAttribute('z-styled', '')
          styleApplied = true
          firstApply = false
        } else if (__DEV__) {
          warn(
            'global-style',
            'More than one z-global-style exist in the document.body. Only the first mounted one will work.'
          )
        }
      })
    })
