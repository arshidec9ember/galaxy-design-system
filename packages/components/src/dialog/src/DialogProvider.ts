...
export const ZDialogProvider = defineComponent({
  name: 'DialogProvider',
  ...
  setup () {
    ...
    function create (options: DialogOptions = {}): DialogReactive {
      ...
      const dialogReactive = reactive({
        ...
        key,
        destroy: () => {
          dialogInstRefs[`z-dialog-${key}`].hide()
        }
      })
    ...
    const typedApi = (
      ...
      return create({ ...options, color: type })
    })
          ...
          omit(dialog, ['destroy', 'style'], {
            ...
            ref: ((inst: DialogInst | null) => {
              if (inst === null) {
                ...
                delete this.dialogInstRefs[`z-dialog-${dialog.key}`]
              } else {
                this.dialogInstRefs[`z-dialog-${dialog.key}`] = inst
              }
            }) as any,
