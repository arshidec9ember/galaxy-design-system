...
import { ZDialogProvider, useDialog, ZDialog, type DialogProps } from '../index'

const Provider = defineComponent({
  render () {
    return <ZDialogProvider>{this.$slots}</ZDialogProvider>
  }
})
...
describe('z-dialog', () => {
  ...
  it('can be a component', () => {
    ...
    expect(document.querySelector('.z-dialog__content')?.textContent).toEqual(
      'success'
    )
    dialog.unmount()
  })
  ...
  it("shouldn't useDialog display button if no text is set", () => {
    ...
    expect(wrapper.find('.z-button').exists()).toEqual(false)
    wrapper.unmount()
  })
  ...
  __FAILED__TESTCASES__ &&
    it('loading', async () => {
      ...
      await nextTick()
      expect(document.querySelector('.z-button__icon')).not.toEqual(null)
      wrapper.unmount()
    })
  ...
  __FAILED__TESTCASES__ &&
    it('should work with `style` option', async () => {
      ...
      await nextTick()
      expect(
        document.querySelector('.z-dialog')?.getAttribute('style')
  ...
  it('should work with `bordered` option', async () => {
    ...
    await nextTick()
    expect(document.querySelector('.z-dialog--bordered')).not.toEqual(null)
    wrapper.unmount()
  })
  ...
  it('should work with `closable` option', async () => {
    ...
    await nextTick()
    expect(document.querySelector('.z-base-icon')).not.toEqual(null)
    wrapper.unmount()
  })
  ...
  it('should work with `icon-placement` option', async () => {
    ...
    await nextTick()
    expect(document.querySelector('.z-dialog--icon-left')).not.toEqual(null)
    wrapperLeft.unmount()
    expect(document.querySelector('.z-dialog--icon-top')).not.toEqual(null)
    wrapperTop.unmount()
  })
  ...
  it('should work with `negative-text` option', async () => {
    ...
    await nextTick()
    expect(document.querySelector('.z-button__content')?.textContent).toBe(
      'test1'
    )
    wrapper.unmount()
  })
