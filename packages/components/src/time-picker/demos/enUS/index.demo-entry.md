...
A time picker helps users select a specific time. Time picker’s format depending on location or need can be customized.

```figma
https://www.figma.com/file/Q8HKejlBjStQ4i9iEGbtIT/Vue-Migration-Items?type=design&node-id=0-1&mode=design&t=YNc8XSlumthoRfAu-0
```

```overview-html
time-picker/demos/enUS/index.html
```
...
| --- | --- | --- | --- |
| actions | `Array<'now'\|'confirm'>` | `['confirm']` | Operations supported by the Time Picker. |
| clearable | `boolean` | `false` | Whether the value is clearable. |
| default-model-value | `number \| null` | `null` | Default value. |
| default-formatted-value | `number \| null` | `undefined` | Default formatted value. |
| disabled | `boolean` | `false` | Disabled state. |
| format | `string` | `'HH:mm:ss'` | Time format. For possible formats see [date-fns.org](https://date-fns.org/v2.23.0/docs/format). |
| formatted-value | `string \| null` | `undefined` | Formatted value. |
...
| placement | `'top-start' \| 'top' \| 'top-end' \| 'right-start' \| 'right' \| 'right-end' \| 'bottom-start' \| 'bottom' \| 'bottom-end' \| 'left-start' \| 'left' \| 'left-end'` | `'bottom-start'` | Time picker panel's placement. |
| show | `boolean` | `undefined` | Whether to show panel |
| size | `'small' \| 'medium' \| 'large'` | `'medium'` | Size. |
| status | `'success' \| 'warning' \| 'error'` | `undefined` | Validation status. |
| time-zone | `string` | `undefined` | Time zone to be used to format the value. It follows [iana time zones](https://www.iana.org/time-zones). You can use `Intl.supportedValuesOf('timeZone')` to check supported values. |
...
| model-value | `number \| null` | `undefined` | Value when being set manually. |
| value-format | `string` | follows `format` | Format of formatted value. |
| on-blur | `() => void` | `undefined` | Callback when the selection box loses focus. |
| on-clear | `() => void` | `undefined` | Callback when value is cleared. |
| on-confirm | `(value: number \| null, formattedValue: string \| null) => void` | `undefined` | Callback when the click confirm button. |
| on-focus | `() => void` | `undefined` | Callback when the selection box gets focus. |
| on-update:formatted-value | `(value: number \| null, timestampValue: number \| null) => void` | `undefined` | Callback when formatted value changes. |
| on-update:show | `(show: boolean) => void` | `undefined` | Callback when panel shows & hides. |
| on-update:model-value | `(value: number \| null, formattedValue: string \| null) => void` | `undefined` | Callback when the value changes. |
