import { h } from 'vue'
import { mount } from '@vue/test-utils'
import { ZButton, ZxButton } from '../index'
import { CashOutline as CashIcon } from '@vicons/ionicons5'
import { ZIcon } from '../../icon'

describe('z-button', () => {
  it('should work with import on demand', () => {
    mount(ZButton)
  })
  it('clickable', async () => {
    const onClick = jest.fn()
    const inst = mount(ZButton, {
      props: {
        onClick
      }
    })
    await inst.trigger('click')
    expect(onClick).toHaveBeenCalled()
    inst.unmount()
  })
  it('keyboard', async () => {
    const onClick = jest.fn()
    const inst = mount(ZButton, {
      props: {
        keyboard: false,
        onClick
      }
    })
    await inst.trigger('click')
    expect(onClick).toBeCalledTimes(1)
    await inst.trigger('keydown.space')
    expect(onClick).toBeCalledTimes(1)
    await inst.trigger('keydown.enter')
    expect(onClick).toBeCalledTimes(1)
    inst.unmount()
  })
  it('disabled', async () => {
    const onClick = jest.fn()
    const inst = mount(ZButton, {
      props: {
        disabled: true,
        onClick
      }
    })
    await inst.trigger('click')
    expect(onClick).not.toHaveBeenCalled()
    inst.unmount()
  })
  it('passed native event & attr tsx type checking', () => {
    ;<div>
      <ZxButton onMousedown={() => {}} />
      <ZxButton formaction="" />
    </div>
  })

  it('should work with `type` prop', () => {
    ;(['button', 'submit', 'reset'] as const).forEach((type) => {
      const wrapper = mount(ZButton, { props: { type } })
      expect(wrapper.find('button').attributes('type')).toContain(type)
      wrapper.unmount()
    })
  })

  it('should work with `full-width` prop', async () => {
    const wrapper = mount(ZButton)
    expect(wrapper.find('.z-button').classes()).not.toContain('z-button--block')

    await wrapper.setProps({ fullWidth: true })
    expect(wrapper.find('.z-button').classes()).toContain('z-button--block')
    wrapper.unmount()
  })

  __FAILED__TESTCASES__ &&
    it('should work with `variant` prop', async () => {
      ;(
        [
          'filled',
          'outlined',
          'light',
          'subtle',
          'outlined',
          'link',
          'text'
        ] as const
      ).forEach((variant) => {
        const wrapper = mount(ZButton, { props: { variant } })
        expect(wrapper.find('button').classes()).toContain(
          `z-button--${variant}-variant`
        )
        wrapper.unmount()
      })
    })

  __FAILED__TESTCASES__ &&
    it('should work with `dashed` prop', async () => {
      const wrapper = mount(ZButton)

      await wrapper.setProps({ borderStyle: 'dashed' })
      expect(wrapper.find('button').classes()).toContain('z-button--dashed')
      wrapper.unmount()
    })

  it('should work with `bordered` prop', async () => {
    const wrapper = mount(ZButton)

    await wrapper.setProps({ variant: 'outlined' })
    expect(wrapper.find('.z-button__border').exists()).toBe(false)
    wrapper.unmount()
  })

  it('should work with `size` prop', async () => {
    ;(['x-small', 'small', 'medium', 'large'] as const).forEach((size) => {
      const wrapper = mount(ZButton, { props: { size } })
      expect(wrapper.find('button').attributes('style')).toMatchSnapshot()
      wrapper.unmount()
    })
  })

  it('should work with `text` prop', () => {
    const wrapper = mount(ZButton, {
      props: {
        text: true
      },
      slots: {
        default: () => 'test'
      }
    })
    expect(wrapper.find('button').attributes('style')).toMatchSnapshot()
    expect(wrapper.find('.z-button__content').element.textContent).toBe('test')
    wrapper.unmount()
  })

  it('should work with `icon` slot', async () => {
    const wrapper = mount(ZButton, {
      slots: {
        icon: () =>
          h(ZIcon, null, {
            default: () => h(CashIcon)
          }),
        default: () => 'test'
      }
    })

    expect(wrapper.find('span').classes('z-button__icon')).toBe(true)
    expect(wrapper.find('div').classes('z-icon-slot')).toBe(true)
    expect(wrapper.find('i').classes('z-icon')).toBe(true)
    expect(wrapper.find('i').classes('z-icon')).toBe(true)
    expect(wrapper.find('.z-button__content').element.textContent).toBe('test')
    expect(wrapper.find('button').element.children[1].textContent).toBe('test')

    await wrapper.setProps({ iconPlacement: 'end' })
    expect(wrapper.find('button').element.children[0].textContent).toBe('test')
    wrapper.unmount()
  })

  __FAILED__TESTCASES__ &&
    it('should work with `tag` prop', () => {
      const wrapper = mount(ZButton, {
        props: {
          text: true,
          tag: 'a',
          href: 'https://www.gds.zeta.tech',
          target: '_blank',
          type: 'submit'
        },
        slots: {
          default: () => 'test'
        }
      })

      expect(wrapper.find('a').attributes('style')).toMatchSnapshot()
      expect(wrapper.find('a').classes()).toContain('z-button--primary-type')
      expect(wrapper.find('a').attributes('type')).toContain('button')
      expect(wrapper.find('a').attributes('disabled')).toContain('false')
      expect(wrapper.find('a').attributes('href')).toContain(
        'https://www.gds.zeta.techch'
      )
      expect(wrapper.find('a').attributes('target')).toContain('_blank')
      expect(wrapper.find('.z-button__content').element.textContent).toBe(
        'test'
      )
      wrapper.unmount()
    })

  __FAILED__TESTCASES__ &&
    it('should work with `shape` ', async () => {
      const wrapper = mount(ZButton, {
        props: {
          circle: true
        },
        slots: {
          icon: () =>
            h(ZIcon, null, {
              default: () => h(CashIcon)
            })
        }
      })
      const circleStyle: string[] = [
        '--z-width: 34px;',
        '--z-padding: none;',
        '--z-border-radius: 34px;'
      ]
      let buttonStyle = wrapper.find('button').attributes('style')
      expect(circleStyle.every((i) => buttonStyle?.includes(i))).toBe(true)

      await wrapper.setProps({ circle: false, round: true })
      const roundStyle: string[] = [
        '--z-width: none;',
        '--z-padding: 0 18px;',
        '--z-border-radius: 34px;'
      ]
      buttonStyle = wrapper.find('button').attributes('style')
      expect(roundStyle.every((i) => buttonStyle?.includes(i))).toBe(true)

      await wrapper.setProps({ circle: false, round: false })
      const defaultStyle: string[] = [
        '--z-width: none;',
        '--z-padding: 0 14px;',
        '--z-border-radius: 3px;'
      ]
      buttonStyle = wrapper.find('button').attributes('style')
      expect(defaultStyle.every((i) => buttonStyle?.includes(i))).toBe(true)
      wrapper.unmount()
    })

  __FAILED__TESTCASES__ &&
    it('should work with `ghost` prop', () => {
      const wrapper = mount(ZButton, {
        props: {
          ghost: true
        },
        slots: {
          default: () => 'test'
        }
      })

      expect(wrapper.find('button').classes()).toContain('z-button--ghost')
      wrapper.unmount()
    })

  it('should work with `icon-placement` prop', async () => {
    const wrapper = mount(ZButton, {
      slots: {
        default: () => 'test',
        icon: () =>
          h(ZIcon, null, {
            default: () => h(CashIcon)
          })
      }
    })
    expect(wrapper.findAll('span')[0].attributes('class')).toContain(
      'z-button__icon'
    )
    expect(wrapper.findAll('span')[1].attributes('class')).toContain(
      'z-text z-text--no-tag'
    )

    await wrapper.setProps({ iconPlacement: 'end' })
    expect(wrapper.findAll('span')[0].attributes('class')).toContain(
      'z-text z-text--no-tag'
    )
    expect(wrapper.findAll('span')[1].attributes('class')).toContain(
      'z-button__content'
    )
    wrapper.unmount()
  })

  it('should work with `loading` prop', () => {
    const wrapper = mount(ZButton, {
      props: {
        loading: true
      },
      slots: {
        default: () => 'test'
      }
    })

    expect(wrapper.find('.z-icon-slot').classes()).toContain('z-base-loading')
    expect(wrapper.find('.z-icon-slot').attributes('aria-label')).toContain(
      'loading'
    )
    wrapper.unmount()
  })

  __FAILED__TESTCASES__ &&
    it('should work with `color` prop', () => {
      const wrapper = mount(ZButton, {
        props: {
          color: '#8a2be2'
        },
        slots: {
          default: () => 'test'
        }
      })

      expect(wrapper.find('button').classes()).toContain('z-button--color')
      const colorStyle: string[] = [
        '--z-color: #8a2be2;',
        '--z-color-disabled: #8a2be2;',
        '--z-ripple-color: #8a2be2;'
      ]
      const buttonStyle = wrapper.find('button').attributes('style')
      expect(colorStyle.every((i) => buttonStyle?.includes(i))).toBe(true)
      wrapper.unmount()
    })

  __FAILED__TESTCASES__ &&
    it('should work with `text-color` prop', () => {
      const wrapper = mount(ZButton, {
        props: {
          'text-color': '#8a2be2'
        },
        slots: {
          default: () => 'test'
        }
      })

      const buttonStyle = wrapper.find('button').attributes('style')
      expect(
        (
          [
            '--z-text-color: #8a2be2;',
            '--z-text-color-disabled: #8a2be2;'
          ] as string[]
        ).every((i) => buttonStyle?.includes(i))
      ).toBe(true)
      wrapper.unmount()
    })
})
