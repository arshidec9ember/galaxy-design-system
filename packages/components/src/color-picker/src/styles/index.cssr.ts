...
export default c([
  ...
  cB('color-picker-panel', `
    margin: 4px 0;
    width: 240px;
    font-size: var(--z-panel-font-size);
  ...
  `, [
    ...
    `, [
      c('&::after', `
        --z-block-size: calc((var(--z-height) - 8px) / 3);
        background-size: calc(var(--z-block-size) * 2) calc(var(--z-block-size) * 2);
        background-position: 0 0, 0 var(--z-block-size), var(--z-block-size) calc(-1 * var(--z-block-size)), calc(-1 * var(--z-block-size)) 0px;  
      `)
    ])
  ]),
