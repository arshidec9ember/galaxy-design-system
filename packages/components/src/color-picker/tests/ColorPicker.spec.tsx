...
describe('z-color-picker', () => {
  it('should work with import on demand', () => {
    mount(ZColorPicker)
  })
  describe('props.modes', () => {
    it('multiple modes', async () => {
      const wrapper = mount(ZColorPicker, {
        attachTo: document.body,
        props: {
          modes: ['hex', 'hsl']
        }
      })
      await wrapper.find('.z-color-picker-trigger').trigger('click')
      expect(document.querySelector('.z-color-picker-panel')).not.toEqual(null)
      const modeDom = document.querySelector('.z-color-picker-input__mode')
  ...
  describe('props.control', () => {
    it('confirm and cancel button', async () => {
      ...
      await wrapper.find('.z-color-picker-trigger').trigger('click')
      expect(document.querySelector('.z-color-picker-panel')).not.toEqual(null)
      expect(document.querySelector('.z-color-picker-pallete')).not.toEqual(
        null
      )
      expect(document.querySelector('.z-button')).not.toEqual(null)
      await wrapper.findComponent(ZButton).trigger('click')
      expect(document.querySelector('.z-color-picker-panel')).toEqual(null)
      expect(document.querySelector('.z-color-picker-pallete')).toEqual(null)
      wrapper.unmount()
    })
  })
  describe('props.swatches', () => {
    it('make the colors legal', async () => {
      ...
      expect(
        document
          .querySelector('.z-color-picker-swatch__fill')
          ?.getAttribute('style')
    ...
    __FAILED__TESTCASES__ &&
      it('output according to mode', async () => {
        ...
        await wrapper.find('.z-color-picker-trigger').trigger('click')
        const swatch = document.querySelector('.z-color-picker-swatch')
        const modeDom = document.querySelector('.z-color-picker-input__mode')
        ...
        while (length && (currentMode = modeDom?.textContent)) {
          ...
          if (actualOutput) {
            expect(onUpdateModelValue).toHaveBeenCalledWith(actualOutput.value)
          }
          ;(modeDom as HTMLElement).click()
          await nextTick()
          length--
...
describe('props.label', () => {
  __FAILED__TESTCASES__ &&
    it('render custom label', async () => {
      ...
      await nextTick()
      expect(
        document.querySelector('.z-color-picker-trigger__value')?.textContent
      ).toEqual('custom')
      wrapper.unmount()
    })
})
...
describe('z-color-picker', () => {
  it('should work with `placement` prop', async () => {
    ;(
      ...
      const wrapper = mount(ZColorPicker, { props: { placement } })
      setTimeout(() => {
        expect(
          document
  ...
  it('should work with `disabled` prop', async () => {
    const wrapper = mount(ZColorPicker)
    expect(wrapper.find('.z-color-picker-trigger').classes()).not.toContain(
      'z-color-picker-trigger--disabled'
    )

    await wrapper.setProps({ disabled: true })
    expect(wrapper.find('.z-color-picker-trigger').classes()).toContain(
      'z-color-picker-trigger--disabled'
    )
    wrapper.unmount()
  })
  ...
  it('should work with `render-label` prop', async () => {
    const wrapper = mount(ZColorPicker)
    expect(wrapper.find('.z-color-picker-trigger__value').text()).not.toBe(
      'test-label'
    )

    await wrapper.setProps({ renderLabel: () => 'test-label' })
    expect(wrapper.find('.z-color-picker-trigger__value').text()).toBe(
      'test-label'
    )
    wrapper.unmount()
  })
