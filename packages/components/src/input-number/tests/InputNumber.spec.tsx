...
import { ZInputNumber } from '../index'
import { ZButton } from '../../button'

describe('z-input-number', () => {
  it('should work with import on demand', () => {
    mount(ZInputNumber)
  })

  it('props.value can be null', () => {
    ;<ZInputNumber modelValue={null} />
  })

  it('should work with `loading` prop', async () => {
    const wrapper = mount(ZInputNumber)
  ...
  it('should work with `show-button` prop', async () => {
    ...
    const wrapper = mount(ZInputNumber)
    expect(wrapper.findComponent(ZButton).exists()).toBe(true)
    await wrapper.setProps({ showButton: false })
    expect(wrapper.findComponent(ZButton).exists()).toBe(false)
  })
  ...
  it('should work with `prefix` & `suffix` slots', async () => {
    ...
    expect(wrapper.find('.z-input-number-suffix').exists()).toBe(true)
    expect(wrapper.find('.z-input-number-suffix').text()).toBe('%')
    wrapper.unmount()
  })
  it('should work with decimal `step`', async () => {
    const wrapper = mount(ZInputNumber, {
      attachTo: document.body,
      props: {
        defaultModelValue: 0.2,
        min: 0,
        step: 0.1
      }
    })
    const input = wrapper.find('.z-input__input-el')
    expect((input.element as HTMLInputElement).value).toEqual('0.2')
    const buttons = wrapper.findAll('.z-input__suffix > button')
    ...
    expect(minusBtn.classes()).toContain('z-button--disabled')
    ...
    expect(addBtn.classes()).toContain('z-button--disabled')
    wrapper.unmount()
  })
  ...
  it('should work with decimal value', async () => {
    ...
    const addBtn = wrapper.findAll('.z-input__suffix > button')[1]
  ...
  it('should work with `updateValueOnInput` prop', async () => {
    ...
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(2.2)
    wrapper.find('input').element.value = ''
    await wrapper.find('input').trigger('input')
    ...
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(22.2)
    ...
    await wrapper.find('input').trigger('blur')
    expect(onUpdateModelValue).toHaveBeenCalledWith(24)
    wrapper.unmount()
  })
  ...
  it('should work with negative decimal value', async () => {
    ...
    wrapper.find('input').element.value = '-.2'
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(-0.2)
    wrapper.unmount()
  })
  ...
  it('should work with decimal value ends at 0', async () => {
    ...
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(2.012)
    wrapper.find('input').element.value = '-2.0'
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledTimes(1)
    wrapper.find('input').element.value = '-2.02'
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(-2.02)
    wrapper.find('input').element.value = '-2.0200'
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledTimes(2)
    wrapper.find('input').element.value = '-2.02003'
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(-2.02003)
    wrapper.unmount()
  })
  ...
  it('should work with integer value ends with 0', async () => {
    ...
    await wrapper.find('input').trigger('input')
    expect(onUpdateModelValue).toHaveBeenCalledWith(10)
    wrapper.unmount()
  })

  it('should work with `status` prop', async () => {
    ;(['success', 'warning', 'error'] as const).forEach((status) => {
      const wrapper = mount(ZInputNumber, { props: { status } })
