...
class ZIndexManager {
  ...
  get elementCount (): any {
    return this.elementZIndex.size
  }

  ensureZIndex (el: HTMLElement, zIndex?: number): any {
    ...
    if (elementZIndex.has(el)) {
      ...
      if (currentZIndex && currentZIndex + 1 === this.nextZIndex) return
    }
  ...
  unregister (el: HTMLElement, zIndex?: number): any {
    const { elementZIndex } = this
    if (elementZIndex.has(el)) {
      elementZIndex.delete(el)
