...
import { type VOverflowInst } from '../src'
...
export default defineComponent({
  setup () {
    ...
    const itemsElRef = computed(() =>
      Array.apply(null, { length: itemCountRef.value } as any).map((_, i) => i)
    )
    return {
    ...
    return h('div', undefined, [
      ...
      h('input', {
        ...
        onInput: (e: InputEvent): void => {
          this.tailValue = (e.target as HTMLInputElement).value
        }
      }),
      h('hr'),
      h(
