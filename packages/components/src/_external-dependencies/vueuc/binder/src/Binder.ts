...
import { type BinderInstance } from './interface'
...
const Binder = defineComponent({
  ...
  setup (props) {
    ...
    // scroll related
    // TODO: we need to fix the document should be pickup by default
    ...
    const onScrollRaf = (): void => {
      followerScrollListeners.forEach((listener) => {
        listener()
      })
    }
    // resize related
    ...
    const onResize = (): void => {
      followerResizeListeners.forEach((listener) => {
        listener()
      })
    }
    onBeforeUnmount(() => {
