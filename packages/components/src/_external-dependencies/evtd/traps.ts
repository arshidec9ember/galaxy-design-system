...
const traps = {
  mousemoveoutside: new WeakMap<EventTarget, WeakMap<any, TrapHandlers>>(),
  clickoutside: new WeakMap<EventTarget, WeakMap<any, TrapHandlers>>()
}
...
function createTrapHandler (
  ...
  console.error(
    `[evtd/create-trap-handler]: name \`${
      name as string
    }\` is invalid. This could be a bug of evtd.`
  )
  return {}
}
