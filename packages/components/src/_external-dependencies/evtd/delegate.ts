...
import { trapOn, trapOff, type TrapEventMap } from './traps'

type Handlers = Set<Handler>

type ElToHandlers = Map<EventTarget, Handlers>

type TypeToHandlers = Record<string, Handlers | undefined>
...
function createDelegate (): Delegate {
  ...
  function spy (event: Event, propName: keyof Event, fn: any): Event {
  ...
  const phaseToTypeToElToHandlers: {
    bubble: Record<string, ElToHandlers | undefined>
    capture: Record<string, ElToHandlers | undefined>
  } = {
    bubble: {},
    capture: {}
  }
