...
import ZInternalLoading from '../../loading'
import ZFocusDetector from '../../focus-detector'
import {
  internalSelectMenuLight,
  type InternalSelectMenuTheme
} from '../styles'
import ZSelectOption from './SelectOption'
import ZSelectGroupHeader from './SelectGroupHeader'
import type {
  RenderLabel,
  Size,
  ...
  setup (props) {
    ...
    const cssVarsRef = computed(() => {
      ...
      return {
        ...
        '--z-color': color,
        '--z-option-font-size': fontSize,
        '--z-group-header-text-color': groupHeaderTextColor,
        '--z-option-check-color': optionCheckColor,
        '--z-option-color-pending': optionColorPending,
        '--z-option-color-active': optionColorActive,
        '--z-option-color-active-pending': optionColorActivePending,
        '--z-option-height': optionHeight,
        '--z-option-opacity-disabled': optionOpacityDisabled,
        '--z-option-text-color': optionTextColor,
        '--z-option-text-color-active': optionTextColorActive,
        '--z-option-text-color-disabled': optionTextColorDisabled,
        '--z-option-text-color-pressed': optionTextColorPressed,
        '--z-option-padding': optionPadding,
        '--z-option-padding-left': getPadding(optionPadding, 'left'),
        '--z-option-padding-right': getPadding(optionPadding, 'right'),
      ...
      >
        {this.loading ? (
          <div class={`${clsPrefix}-base-select-menu__loading`}>
            <ZInternalLoading clsPrefix={clsPrefix} strokeWidth={20} />
          </div>
        ) : !this.empty ? (
          <ZScrollbar
            ref="scrollbarRef"
        ...
        {resolveWrappedSlot(
          ...
          (children) =>
            children && [
              ...
              <ZFocusDetector onFocus={this.onTabOut} key="focus-detector" />
            ]
        )}
      </div>
    )
