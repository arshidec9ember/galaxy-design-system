import { c, cB, cE, cM, cNotM } from '../../../../_utils/cssr'
import { fadeInScaleUpTransition } from '../../../../_styles/transitions/fade-in-scale-up.cssr'

// --z-loading-color
// --z-loading-size
// --z-option-padding-right
export default cB('base-select-menu', `
  line-height: 1.5;
  outline: none;
  z-index: 0;
  position: relative;
  border-radius: var(--z-border-radius);
  background-color: var(--z-color);
  transition:
    background-color .3s var(--z-bezier),
    box-shadow .3s var(--z-bezier);
`, [
  cB('scrollbar', `
    max-height: var(--z-height);
  `),
  cB('virtual-list', `
    max-height: var(--z-height);
  `),
  cB('base-select-option', `
    min-height: var(--z-option-height);
    font-size: var(--z-option-font-size);
    display: flex;
    align-items: center;
  `, [
    cE('content', `
      z-index: 1;
      white-space: nowrap;
      text-overflow: ellipsis;
      overflow: hidden;
    `)
  ]),
  cB('base-select-group-header', `
    min-height: var(--z-option-height);
    font-size: .93em;
    display: flex;
    align-items: center;
  `),
  cB('base-select-menu-option-wrapper', `
    position: relative;
    width: 100%;
  `),
  cE('loading, empty', `
    display: flex;
    padding: 12px 32px;
    flex: 1;
    justify-content: center;
  `),
  cE('loading', `
    color: var(--z-loading-color);
    font-size: var(--z-loading-size);
  `),
  cE('action', `
    padding: 8px var(--z-option-padding-left);
    font-size: var(--z-option-font-size);
    transition: 
      color .3s var(--z-bezier),
      border-color .3s var(--z-bezier);
    border-top: 1px solid var(--z-action-divider-color);
    color: var(--z-action-text-color);
  `),
  cB('base-select-group-header', `
    position: relative;
    cursor: default;
    padding: var(--z-option-padding);
    color: var(--z-group-header-text-color);
  `),
  cB('base-select-option', `
    cursor: pointer;
    position: relative;
    padding: var(--z-option-padding);
    transition:
      color .3s var(--z-bezier),
      opacity .3s var(--z-bezier);
    box-sizing: border-box;
    color: var(--z-option-text-color);
    opacity: 1;
  `, [
    cM('show-checkmark', `
      padding-right: calc(var(--z-option-padding-right) + 20px);
    `),
    c('&::before', `
      content: "";
      position: absolute;
      left: 0px;
      right: 0px;
      top: 0;
      bottom: 0;
      transition: background-color .3s var(--z-bezier);
    `),
    c('&:active', `
      color: var(--z-option-text-color-pressed);
    `),
    cM('grouped', `
      padding-left: calc(var(--z-option-padding-left) * 1.5);
    `),
    cM('pending', [
      c('&::before', `
        background-color: var(--z-option-color-pending);
      `)
    ]),
    cM('selected', `
      color: var(--z-option-text-color-active);
    `, [
      c('&::before', `
        background-color: var(--z-option-color-active);
      `),
      cM('pending', [
        c('&::before', `
          background-color: var(--z-option-color-active-pending);
        `)
      ])
    ]),
    cM('disabled', `
      cursor: not-allowed;
    `, [
      cNotM('selected', `
        color: var(--z-option-text-color-disabled);
      `),
      cM('selected', `
        opacity: var(--z-option-opacity-disabled);
      `)
    ]),
    cE('check', `
      font-size: 16px;
      position: absolute;
      right: calc(var(--z-option-padding-right) - 4px);
      top: calc(50% - 7px);
      color: var(--z-option-check-color);
      transition: color .3s var(--z-bezier);
    `, [
      fadeInScaleUpTransition({
        enterScale: '0.5'
      })
    ])
  ])
])
