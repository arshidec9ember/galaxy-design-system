...
The text inputs are part of a form fields group of components that are used to input, display, and edit characters. They can be used for entering numeric and alphanumeric values.
...
| count-graphemes | `(value: string) => number` | `undefined` | Count graphemes of input value. If it's set, native `maxlength` and `minlength` won't be used. |
| default-model-value | `string \| [string, string] \| null` | `null` | Default value when not manually set. |
...
| render-count | `(props: { value: string }) => void` | `undefined` | Render function of word count. |
...
| type | `'text' \| 'password' \| 'textarea' \| 'number'` | `'text'` | Input type. |
| model-value | `string \| [string, string] \| null` | `undefined` | Manually set the input value. When `pair` is `true`, this is an array. |
...
| on-update:model-value | `(value: string \| [string, string]) => void` | `undefined` | Callback triggered when the input value changes. |
...
| password-invisible-icon | `()` | Password toggle icon when password is invisible. |
| password-visible-icon | `()` | Password toggle icon when password is visible. |
...
| scrollTo | `(options: { left?: number, top?: number, behavior?: 'auto' \| 'smooth' }) => void` | Scroll To. |
