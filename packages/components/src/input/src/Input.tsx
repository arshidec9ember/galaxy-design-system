...
import { off, on } from '../../_external-dependencies/evtd'
...
export const inputProps = {
  ...
  onInput: [Function, Array] as PropType<OnUpdateModelValue>,
  onFocus: [Function, Array] as PropType<MaybeArray<(e: FocusEvent) => void>>,
  onBlur: [Function, Array] as PropType<MaybeArray<(e: FocusEvent) => void>>,
  onClick: [Function, Array] as PropType<MaybeArray<(e: MouseEvent) => void>>,
  onChange: [Function, Array] as PropType<OnUpdateModelValue>,
...
export default defineComponent({
  ...
  setup (props) {
    ...
    function doUpdateValue (value: string | [string, string]): void {
      const { onInput, type } = props
      const { nTriggerFormInput } = formItem
      let updatedValue: string | [string, string] | number = value
      if (type?.toLowerCase()?.trim() === 'number' && value) {
        updatedValue = Number(value)
      }
      controlledValueRef.value = updatedValue as string | [string, string]
      if (onInput) call(onInput as OnUpdateModelValueImpl, updatedValue)
      uncontrolledValueRef.value = updatedValue
      nTriggerFormInput()
    }
    ...
    function syncMirror (value: string | number | null): void {
      const { type, pair, autosize } = props
      if (!pair && autosize) {
        if (type === 'textarea') {
          const { value: textareaMirrorEl } = textareaMirrorElRef
          if (textareaMirrorEl) {
            textareaMirrorEl.textContent = String(value ?? '') + '\r\n'
          }
        } else {
          const { value: inputMirrorEl } = inputMirrorElRef
          if (inputMirrorEl) {
            if (value) {
              inputMirrorEl.textContent = String(value)
            } else {
    ...
    const cssVarsRef = computed(() => {
      ...
      return {
        ...
        '--z-count-text-color': countTextColor,
        '--z-count-text-color-disabled': countTextColorDisabled,
        '--z-color': color,
        '--z-font-size': fontSize,
        '--z-border-radius': borderRadiusLarge,
        '--z-height': height,
        '--z-padding-left': paddingLeft,
        '--z-padding-right': paddingRight,
        '--z-text-color': textColor,
        '--z-caret-color': caretColor,
        '--z-text-decoration-color': textDecorationColor,
        '--z-border': border,
        ...
        '--z-line-height-textarea': lineHeightTextarea,
        '--z-color-disabled': colorDisabled,
        '--z-color-focus': colorFocus,
        '--z-text-color-disabled': textColorDisabled,
        '--z-box-shadow-focus': boxShadowFocus,
        '--z-loading-color': loadingColor,
        // form warning
        '--z-caret-color-warning': caretColorWarning,
        '--z-color-focus-warning': colorFocusWarning,
        ...
        '--z-loading-color-warning': loadingColorWarning,
        // form error
        '--z-caret-color-error': caretColorError,
        '--z-color-focus-error': colorFocusError,
        ...
        '--z-loading-color-error': loadingColorError,
        // clear-button
        ...
        '--z-suffix-text-color': suffixTextColor,
        '--z-input-background-color': backgroundColor,
        '--z-input-prefix-background-color': inputPrefixBackgroudColor
      }
    })
