...
export default c([
  ...
  `, [
    ...
    `, [
      cNotM('scrollable', [
        cNotM('show-header-or-footer', 'padding: var(--z-padding);')
      ])
    ]),
    cE('header', `
  ...
  `, [
    ...
    c('&.popover-transition-enter-active', `
      transition:
        ...
        background-color .3s var(--z-bezier),
        color .3s var(--z-bezier),
        opacity .15s var(--z-bezier-ease-out),
        transform .15s var(--z-bezier-ease-out);
    `),
    c('&.popover-transition-leave-active', `
      transition:
        ...
        background-color .3s var(--z-bezier),
        color .3s var(--z-bezier),
        opacity .15s var(--z-bezier-ease-in),
        transform .15s var(--z-bezier-ease-in);
    `)
  ]),
  cB('tooltip', [
    cB('popover-arrow-wrapper', `
...
function placementStyle (
  ...
  const sizeStyle = ['top', 'bottom'].includes(position)
    ? 'height: var(--z-space-arrow);'
    : 'width: var(--z-space-arrow);'
  return c(`[v-placement="${placement}"] >`, [
    cB('popover-shared', `
      margin-${oppositePlacement[position]}: var(--z-space);
    `, [
      cM('show-arrow', `
        margin-${oppositePlacement[position]}: var(--z-space-arrow);
      `),
      cM('overlap', `
        margin: 0;
      `),
