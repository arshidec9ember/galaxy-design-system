...
export const tabsProps = {
  ...
  defaultModelValue: [String, Number] as PropType<string | number>,
  trigger: {
...
export default defineComponent({
  ...
  setup (props, { slots }) {
    if (__DEV__) {
      watchEffect(() => {
        ...
        if (props.onActiveNameChange !== undefined) {
          warnOnce(
            'tabs',
            '`on-active-name-change` is deprecated, please use `on-update:model-value` instead.'
          )
        }
      })
    }
    ...
    const tabWrapperStyleRef = computed(() => {
      if (!props.justifyContent || props.variant === 'card') return undefined
      return {
        display: 'flex',
    ...
    function updateCurrentBarStyle (): void {
      ...
      if (tabEl) {
        ...
        const { value: barEl } = barElRef
        barEl && (barEl.style.maxWidth = '0px')
      }
    }
    ...
    function doUpdateValue (panelName: string | number): void {
      ...
      if (onActiveNameChange) {
        call(onActiveNameChange as OnUpdateModelValueImpl, panelName)
      }
      if (onUpdateModelValue) {
        call(onUpdateModelValue as OnUpdateModelValueImpl, panelName)
      }
      if (_onUpdateModelValue) {
        call(_onUpdateModelValue as OnUpdateModelValueImpl, panelName)
      }
    ...
    const cssVarsRef = computed(() => {
      ...
      const sizeType = `${size}${variantSuffix}` as const
      const {
        self: {
          barColor,
          closeIconColor,
          ...
          [createKey('tabTextColor', variant)]: tabTextColor,
          [createKey('tabTextColorActive', variant)]: tabTextColorActive,
          [createKey('tabTextColorHover', variant)]: tabTextColorHover,
          [createKey('tabTextColorDisabled', variant)]: tabTextColorDisabled,
      ...
      return {
        ...
        '--z-color-segment': colorSegment,
        '--z-bar-color': barColor,
        '--z-tab-font-size': tabFontSize,
        '--z-tab-text-color': tabTextColor,
        '--z-tab-text-color-active': tabTextColorActive,
        '--z-tab-icon-color': tabIconColor,
        '--z-tab-text-color-disabled': tabTextColorDisabled,
        '--z-tab-text-color-hover': tabTextColorHover,
        '--z-pane-text-color': paneTextColor,
        '--z-tab-border-color': tabBorderColor,
        '--z-tab-border-radius': tabBorderRadius,
        ...
        '--z-tab-color': tabColor,
        '--z-tab-font-weight': tabFontWeight,
        '--z-tab-font-weight-active': tabFontWeightActive,
        '--z-tab-padding': tabPadding,
        '--z-tab-padding-vertical': tabPaddingVertical,
        '--z-tab-gap': tabGap,
        '--z-tab-gap-vertical': tabGapVertical,
        '--z-pane-padding-left': getPadding(panePadding, 'left'),
        '--z-pane-padding-right': getPadding(panePadding, 'right'),
        '--z-pane-padding-top': getPadding(panePadding, 'top'),
        '--z-pane-padding-bottom': getPadding(panePadding, 'bottom'),
        '--z-font-weight-strong': fontWeightStrong,
        '--z-tab-color-segment': tabColorSegment
      }
    })
    ...
    const themeClassHandle = inlineThemeDisabled
      ? useThemeClass(
        'tabs',
        computed(() => {
          return `${compitableSizeRef.value[0]}${props.variant[0]}`
        }),
        cssVarsRef,
        props
      )
  ...
  render () {
    const {
      ...
      paneWrapperClass,
      paneWrapperStyle,
      $slots: { default: defaultSlot, start: startSlot, end: endSlot }
    } = this
    ...
    return (
      <div
        class={[
          ...
          this.themeClass,
          `${mergedClsPrefix}-tabs--${variant}-type`,
      ...
      >
        <div
          class={[
            ...
            // style.
            `${mergedClsPrefix}-tabs-nav--${variant}-type`,
        ...
        >
          {resolveWrappedSlot(
            ...
            (children) =>
              children && (
                <div class={`${mergedClsPrefix}-tabs-nav__start`}>
                  {children}
                </div>
              )
          )}
          ...
          {resolveWrappedSlot(
            ...
            (children) =>
              children && (
                <div class={`${mergedClsPrefix}-tabs-nav__end`}>{children}</div>
              )
          )}
        </div>
        {showPane &&
