# Upload
A file uploader component is used to upload documents and images from the computer to the application.

```figma
https://www.figma.com/file/t4dWFndlBl46TnLIH2n1dJ/%F0%9F%94%AE-GDS-v2.11.0---Foundation-%2B-Components?type=design&node-id=16501-42364&mode=design&t=5Sq8fIvuNxEemofa-0
```

```overview-html
upload/demos/enUS/index.html
```
...
| accept | `string` | `undefined` | The accept type of upload. See <z-a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/file#accept" target="_blank">accept</z-a>. |  |
...
| custom-request | `(options: UploadCustomRequestOptions) => void` | `undefined` | Customize upload request. For types, see <z-a href="#UploadCustomRequestOptions-Type">UploadCustomRequestOptions</z-a> |  |
...
| image-group-props | `ImageGroupProps` | `undefined` | Props of `z-image` inside upload. See [ImageGroup Props](image#ImageGroup-Props). | 2.24.0 |
...
| response-type | `'' \| 'arraybuffer' \| 'blob' \| 'document' \| 'json' \| 'text'` | `''` | Response type of `XMLHttpRequest` used by `z-upload` | 2.33.3 |
...
| Name | Parameters | Description |
| --- | --- | --- |
| default | `()` | The placeholder of the upload dragger; For an example see <z-a href="#drag.vue">Drag to Upload Demo</z-a>. |
