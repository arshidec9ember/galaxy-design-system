...
export const NotificationEnvironment = defineComponent({
  ...
  setup (props) {
    ...
    const progressPercentageRef = ref(100)
    ...
    function showWithTimer (): void {
      const { duration } = props
      if (duration) {
        remainingTime.value = remainingTime.value
          ? remainingTime.value
          : duration

        timerId = window.setInterval(() => {
          remainingTime.value -= 100
          progressPercentageRef.value = (remainingTime.value / duration) * 100
    ...
    return {
      show: showRef,
      hide,
      progressPercentage: progressPercentageRef,
      handleClose,
      handleAfterLeave,
      handleLeave,
      handleBeforeLeave,
