...
import { omit, useRootNode } from '../../_utils'
  ...
  setup (props) {
    ...
    const api: MessageApiInjection = {
      create (content: ContentType, options?: MessageOptions) {
        return create(content, { color: 'neutral', ...options })
      },
      info (content: ContentType, options?: MessageOptions) {
        return create(content, { ...options, color: 'info' })
      },
      success (content: ContentType, options?: MessageOptions) {
        return create(content, { ...options, color: 'success' })
      },
      warning (content: ContentType, options?: MessageOptions) {
        return create(content, { ...options, color: 'warning' })
      },
      error (content: ContentType, options?: MessageOptions) {
        return create(content, { ...options, color: 'error' })
      },
      loading (content: ContentType, options?: MessageOptions) {
        return create(content, { ...options, color: 'loading' })
      },
      destroyAll
    }
    ...
    function create (
      content: ContentType,
      options: MessageOptions & { color: MessageType }
    ): MessageReactive {
      const key = createId()
      ...
      if (maxItems && messageListRef.value.length >= maxItems) {
