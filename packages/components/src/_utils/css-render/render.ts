...
import {
  type CNode,
  type CProperties,
  type CssRenderInstance,
  type CProperty,
  type CPlainProperties,
  type CRenderProps,
  type CNodeChildren,
  type CRenderOption,
  type CSelectorPath
...
function unwrapProperty (prop: CProperty, indent: string = '  '): string {
...
function loopCNodeListWithCallback (
  children: CNodeChildren,
  options: CRenderOption,
  callback: (node: CNode | string) => any
): void {
  /* istanbul ignore if */
  if (!children) return
