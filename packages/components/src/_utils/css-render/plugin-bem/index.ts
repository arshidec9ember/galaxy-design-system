...
function plugin (options?: BEMPluginOptions): CssRenderBemPlugin {
      ...
      $ ({ context, props }) {
        ...
        function elementToSelector (el?: string): string {
          return modifiers
            .map(
              (modifier) =>
                `&${props?.bPrefix || _bPrefix}${context.bem.b as string}${
  ...
  const cB = ((...args: any[]) =>
    c(b(args[0]), args[1], args[2])) as createCNode<string>
  const cE = ((...args: any[]) =>
    c(e(args[0]), args[1], args[2])) as createCNode<string>
  const cM = ((...args: any[]) =>
    c(m(args[0]), args[1], args[2])) as createCNode<string>
  const cNotM = ((...args: any[]) =>
    c(notM(args[0]), args[1], args[2])) as createCNode<string>
