...
| model-value | `string \| null` | `undefined` | Manually set input value. |
| default-model-value | `string` | `''` | Default value when the value is not manually set. |
...
| placement | `'top-start' \| 'top' \| 'top-end' \| 'right-start' \| 'right' \| 'right-end' \| 'bottom-start' \| 'bottom' \| 'bottom-end' \| 'left-start' \| 'left' \| 'left-end'` | `'bottom-start'` | Selection panel's placement . |
| render-option-label | `(option: MentionOption) => VNodeChild` | `undefined` | Options' labels render function. |
...
| on-update:show | `(show: boolean) => void` | `undefined` | Callback when the selection panel of mentions is shown or hidden. |
| on-update:model-value | `(value: string) => void` | `undefined` | Triggered when the input box value is updated. |
