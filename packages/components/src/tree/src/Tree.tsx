...
import { ZxScrollbar } from '../../_internal'
...
export const treeSharedProps = {
  ...
  renderOptionExpandIcon: Function as PropType<RenderSwitcherIcon>,
...
export const treeProps = {
  ...
  variant: {
    type: String as PropType<'underline'>,
    default: 'undefined'
  },
  checkboxPlacement: {
  ...
  renderOptionStart: Function as PropType<RenderPrefix>,
  renderOptionEnd: Function as PropType<RenderSuffix>,
...
export default defineComponent({
  ...
  setup (props) {
    ...
    watch(expandedNonLoadingKeysRef, (value, prevValue) => {
      ...
      const nodeHeight = depx(cssVarsRef.value['--z-node-height'])
    ...
    provide(treeInjectionKey, {
      ...
      patternRef: toRef(props, 'pattern'),
      ...
      fNodesRef,
      pendingNodeKeyRef,
      variantRef: toRef(props, 'variant'),
      showLineRef: toRef(props, 'showLine'),
      showEllipsisRef: toRef(props, 'showEllipsis'),
      disabledFieldRef: toRef(props, 'disabledField'),
      internalScrollableRef: toRef(props, 'internalScrollable'),
      internalCheckboxFocusableRef: toRef(props, 'internalCheckboxFocusable'),
      internalTreeSelect: props.internalTreeSelect,
      renderLabelRef: toRef(props, 'renderOptionLabel'),
      renderPrefixRef: toRef(props, 'renderOptionStart'),
      renderSuffixRef: toRef(props, 'renderOptionEnd'),
      renderSwitcherIconRef: toRef(props, 'renderOptionExpandIcon'),
    ...
    const cssVarsRef = computed(() => {
      ...
      const gdsTokens = mergedGDSTokenRef?.value
      const {
        ...
        self: {
          ...
          dropMarkColor,
          nodeHighlightBottomColor,
          borderWidth,
          lineHeight,
          dividerColor,
          selectedNodeHoverColor,
          [createKey('fontSize', size)]: fontSize,
          [createKey('nodePaddingSize', size)]: nodePadding,
          [createKey('nodeWrapperPaddingSize', size)]: nodeWrapperPadding
        }
      } = themeRef.value
      const lineOffsetTop = getPadding(
        pxfy(depx(nodePadding) + depx(nodeWrapperPadding)),
        'top'
      )
      const lineOffsetBottom = getPadding(
        pxfy(depx(nodePadding) + depx(nodeWrapperPadding)),
        'bottom'
      )
      const resolvedFontSize = Number(getResolvedValueFromToken(gdsTokens, fontSize)) || 0;
      const nodeContentHeight = pxfy(
        depx(resolvedFontSize) * depx(lineHeight)
      )
      const nodeHeight = pxfy(
        depx(nodeContentHeight) + depx(lineOffsetTop) + depx(lineOffsetBottom)
      )
      return {
        ...
        '--z-node-border-radius': nodeBorderRadius,
        '--z-node-color-active': nodeColorActive,
        '--z-node-color-hover': nodeColorHover,
        '--z-node-color-pressed': nodeColorPressed,
        '--z-node-text-color': nodeTextColor,
        '--z-node-text-color-hover': nodeTextColorHover,
        '--z-node-text-color-active': nodeTextColorActive,
        '--z-node-text-color-disabled': nodeTextColorDisabled,
        '--z-node-text-bottom-highlight-color': nodeHighlightBottomColor,
        '--z-drop-mark-color': dropMarkColor,
        '--z-node-wrapper-padding': nodeWrapperPadding,
        '--z-node-padding': nodePadding,
        '--z-line-offset-top': `-${lineOffsetTop}`,
        '--z-line-offset-bottom': `-${lineOffsetBottom}`,
        '--z-node-content-height': nodeContentHeight,
        '--z-line-height': lineHeight,
        '--z-line-width': borderWidth,
        '--z-node-height': nodeHeight,
        '--z-divider-color': dividerColor,
        '--z-selected-node-hover-color': selectedNodeHoverColor
      }
    })
            ...
            default: () => {
              ...
              return (
                <VVirtualList
                  ...
                  items={this.fNodes}
                  // TODO: Default min size value should be 1rem. Refer it from common font size
                  itemSize={depx(cssVars?.['--z-node-height'] || '14px')}
