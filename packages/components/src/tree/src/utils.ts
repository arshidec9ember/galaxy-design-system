...
/**
 * Resolves a CSS variable value to its computed pixel value.
 * @param gdsTokens - Object containing GDS token definitions
 * @param value - CSS variable string (e.g., 'var(--gds-token-name)')
 * @param rootFontSize - Base font size in pixels (default: 16)
 * @returns Computed pixel value as number, or original value if not a CSS variable
 */
export const getResolvedValueFromToken = (
  gdsTokens: Record<string, string>,
  value: string,
  rootFontSize: number = 16
): number | string => {
  if (gdsTokens === undefined) return value

  if (typeof value === 'string' && value.trim().startsWith('var')) {
    const cssVar = /var\((.*?)\)/
    const match = value.match(cssVar)
    const tokenName = match?.[1]?.trim()

    if (!tokenName) return value

    const remValue = gdsTokens[tokenName] || ''
    const rem =
      typeof remValue === 'string'
        ? parseFloat(remValue.replace('rem', ''))
        : remValue
