import { c, cB, cNotM } from '../../../_utils/cssr'
import { fadeInTransition } from '../../../_styles/transitions/fade-in.cssr'
import { fadeInScaleUpTransition } from '../../../_styles/transitions/fade-in-scale-up.cssr'

// vars:
// --z-toolbar-icon-color
// --z-toolbar-color
// --z-toolbar-border-radius
// --z-toolbar-box-shadow
// --z-bezier
export default c([
  c('body >', [
    cB('image-container', 'position: fixed;')
  ]),
  cB('image-preview-container', `
    position: fixed;
    left: 0;
    right: 0;
    top: 0;
    bottom: 0;
    display: flex;
  `),
  cB('image-preview-overlay', `
    z-index: -1;
    position: absolute;
    left: 0;
    right: 0;
    top: 0;
    bottom: 0;
    background: rgba(0, 0, 0, .3);
  `, [
    fadeInTransition()
  ]),
  cB('image-preview-toolbar', `
    z-index: 1;
    position: absolute;
    left: 50%;
    transform: translateX(-50%);
    border-radius: var(--z-toolbar-border-radius);
    height: 48px;
    bottom: 40px;
    padding: 0 12px;
    background: var(--z-toolbar-color);
    box-shadow: var(--z-toolbar-box-shadow);
    color: var(--z-toolbar-icon-color);
    transition: color .3s var(--z-bezier);
    display: flex;
    align-items: center;
  `, [
    cB('base-icon', `
      padding: 0 8px;
      font-size: 28px;
      cursor: pointer;
    `),
    fadeInTransition()
  ]),
  cB('image-preview-wrapper', `
    position: absolute;
    left: 0;
    right: 0;
    top: 0;
    bottom: 0;
    display: flex;
    pointer-events: none;
  `, [
    fadeInScaleUpTransition()
  ]),
  cB('image-preview', `
    user-select: none;
    -webkit-user-select: none;
    pointer-events: all;
    margin: auto;
    max-height: calc(100vh - 32px);
    max-width: calc(100vw - 32px);
    transition: transform .3s var(--z-bezier);
  `),
  cB('image', `
    display: inline-flex;
    max-height: 100%;
    max-width: 100%;
  `, [
    cNotM('preview-disabled', `
      cursor: pointer;
    `),
    c('img', `
      border-radius: inherit;
    `)
  ])
])
