...
<z-alert title="Warning" color="warning" :bordered="false">
  The transfer component is deprecated. It won't have any new feature and will be removed in the next major version. It's recommended to use new  <router-link to="transfer" custom v-slot="{ href, navigate }">
    <z-a :href="href" @click="navigate">Transfer</z-a>
  </router-link>.
</z-alert>
...
| --- | --- | --- | --- |
| default-model-value | `Array<string \| number> \| null` | `null` | Default value. |
...
| model-value | `Array<string \| number> \| null` | `undefined` | Value when being set manually. |
| on-update:model-value | `(value: Array<string \| number>) => void` | `undefined` | Callback when the value changes. |
