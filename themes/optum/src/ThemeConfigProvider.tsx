...
const THEME_ID = 'zeta-gds/optum-theme'

const optumComponentOptions = {
  Pagination: {
    inputSize: 'medium'
  },
  DatePicker: {
  ...
  render () {
    ...
    return (
      <ZConfigProvider
        ...
        style={gdsTokens}
        gdsTokens={gdsTokens}
        class={`optum-${String(themeName)}-theme`}
        themeOverrides={
          themeOverrides
            ? merge({}, optumThemeOverrides, themeOverrides)
            : optumThemeOverrides
        }
        componentOptions={
          componentOptions
            ? merge({}, optumComponentOptions, componentOptions)
            : optumComponentOptions
        }
